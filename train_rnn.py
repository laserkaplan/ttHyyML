...
def train_leptonic():
    ...
    sig_jets = np.load('arrays/ttH_leptonic_jets.npy')
    bkg_jets = np.load('arrays/data_IsoPID_leptonic_jets.npy')

    sig_jets = utils.restrictSample(sig_jets, len(bkg_jets), args.signal)
    
    # load lepmet data
    print('Loading lepmet data.')

    sig_lepmets = np.load('arrays/ttH_leptonic_lepmets.npy')
    bkg_lepmets = np.load('arrays/data_IsoPID_leptonic_lepmets.npy')

    sig_lepmets = utils.restrictSample(sig_lepmets, len(bkg_lepmets), args.signal)
    
    # load photon data
    print('Loading photon data.')

    sig_photons = np.load('arrays/ttH_leptonic_photons.npy')
    bkg_photons = np.load('arrays/data_IsoPID_leptonic_photons.npy')

    sig_photons = utils.restrictSample(sig_photons, len(bkg_photons), args.signal)

    # scale branches

    pts = np.concatenate((sig_jets[:, :, 0].flatten(), bkg_jets[:, :, 0].flatten(), sig_photons[:, :, 0].flatten(), bkg_photons[:, :, 0].flatten(), sig_lepmets[:, :, 0].flatten(), bkg_lepmets[:, :, 0].flatten()))
    mask_pts = pts != -999
    scaler_pt = StandardScaler()
    scaler_pt.fit(pts[mask_pts])
    
    Es = np.concatenate((sig_jets[:, :, 3].flatten(), bkg_jets[:, :, 3].flatten(), sig_photons[:, :, 3].flatten(), bkg_photons[:, :, 3].flatten(), sig_lepmets[:, :, 3].flatten(), bkg_lepmets[:, :, 3].flatten()))
    mask_Es = Es != -999
    ...
    pts_sig = sig_lepmets[:, :, 0].flatten()
    pts_sig = scaler_pt.transform(pts_sig)
    sig_lepmets[:, :, 0] = pts_sig.reshape(-1, 2)
    
    Es_sig = sig_lepmets[:, :, 3].flatten()
    Es_sig = scaler_E.transform(Es_sig)
    sig_lepmets[:, :, 3] = Es_sig.reshape(-1, 2)
    
    pts_bkg = bkg_lepmets[:, :, 0].flatten()
    pts_bkg = scaler_pt.transform(pts_bkg)
    bkg_lepmets[:, :, 0] = pts_bkg.reshape(-1, 2)
    
    Es_bkg = bkg_lepmets[:, :, 3].flatten()
    Es_bkg = scaler_E.transform(Es_bkg)
    bkg_lepmets[:, :, 3] = Es_bkg.reshape(-1, 2)

    # save scalers for later use
    scalers = {'pt': scaler_pt, 'E': scaler_E}
    pickle.dump(scalers, open('scalers_leptonic.p', 'wb'))

    '''
    # pad photon and lepmet arrays with dummy b-tag values
    ...
    sig_lepmets = np.concatenate((sig_lepmets, np.ones((sig_lepmets.shape[0], sig_lepmets.shape[1], 1))), axis=2)
    bkg_lepmets = np.concatenate((bkg_lepmets, np.ones((bkg_lepmets.shape[0], bkg_lepmets.shape[1], 1))), axis=2)
    '''

    # make full timestep arrays
    sig = np.concatenate((sig_photons, sig_lepmets, sig_jets), axis=1)
    bkg = np.concatenate((bkg_photons, bkg_lepmets, bkg_jets), axis=1)
    ...
    mc = ModelCheckpoint('models/model_leptonic_rnn_temp.h5', monitor='val_loss', save_best_only=True)
    try:
        model.fit(train, y_train_cat, epochs=1000, batch_size=32, validation_data=(val, y_val_cat), callbacks=[es, mc])
    ...
    if args.save:
        print('Saving model')
        model.save_weights('models/model_leptonic_rnn.h5')
